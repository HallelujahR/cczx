...
@section('title')
<title>{{ $user->name}}的个人中心_详细信息[传承网]</title>
@endsection
        ...
        <div id="personal_right">
            ...
            <div id="myTabContent" class="tab-content">
               <div class="tab-pane fade in active" id="card">
                  ...
                  <hr>
            @if($user->card->status == 0 || $user->card->status == 2)
                @if($user->card->status == 2)
                    <div style="text-align:center;font-size:18px;"><b style="color:red">审核失败原因：</b><span style="color:#00A4C5;">{{ $user->card->info }}</div>
                ...
                @endif
                  ...
                  <!-- 表单 -->
                  <form class="layui-form" action="{{ action('PerfectController@createCard') }}" method="post" style="margin-top:30px" enctype="multipart/form-data">
                      {{ csrf_field() }}
                         ...
                         <tbody id="bank-line">
                           @foreach($banks as $k => $v)
                            <tr id="bk{{$v['id']}}">
                               ...
                               <td>{{$v['tel']}}</td>
                               <td>
                                 <button bankid="{{$v['id']}}" type="button" class="btn btn-primary bank-edit" data-toggle="modal" data-target="#myModal">
                                   修改
                                 </button>
                                 <button bankid="{{$v['id']}}"  type="button" class="btn btn-danger bank-del">删除</button>
                               </td>
                            </tr>
                            @endforeach

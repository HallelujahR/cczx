...
@section('content')
    ...
    <div class="htbox">
        ...
        <div class="xml">
            <div class="xmll">
                <a href="{{ action('TopicController@publish',['type'=>$post->cate->id]) }}" id="fbtz">发表话题</a>
            </div>
            <div class="xmlr">
                <p>
            ...
            <div class="reply">
                <div class="reply_down">
                    ...
                    <div class="reply_content">
                        ...
                        <div class="user_detail_x">
                            <div class="user_detail_x_line">认证状态 :
                                @if($reply->user->role->role == '认证会员')
                                    <span>{{$reply->user->role->role}} <span class="rzzt">「此用户已通过实名认证，地址，银行，姓名均认证成功」</span></span>
                                @elseif($reply->user->role->role == '普通会员')
                                    <span>{{$reply->user->role->role}} 「此用户还未实名认证」</span>
                                @elseif($reply->user->role->role == '管理员')
                                    <span>{{$reply->user->role->role}}</span>
                                @endif
                            </div>
        ...
        <div style="float:left;width:100%;padding:0px;">
            @if(Auth::check() && Auth::user())
                <div id="plArticle">
                    ...
                    <div>
                        <form id="form" class="form-horizontal" action="{{ action('TopicController@storeReply') }}"
                              method="post">
                            {{ csrf_field() }}

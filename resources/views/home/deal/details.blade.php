...
@section('content')
    ...
    <div class="htbox">
                        ...
                        <div class="owner_detail">
                            <div class="owner_user_headpic">
                                <a href="/personal/{{$post->user->id}}">
                                    <img src="{{$post->user->avatar}}"  width="90" height="90"
                            ...
                            <div class="owner_user_detail">信用积分 : <a href="{{action('DealController@credit',$post->user->id)}}" target="_blank">点击查看</a></div>
                            ...
                            <div class="owner_user_detail">
                                好评：{{$post->user->mark->good}}
                                中评：{{$post->user->mark->commonly}}
                                差评：{{$post->user->mark->bad}}
                            </div>
                        </div>
                        <div class="owner_content">
                            <div class="res" style="min-height:200px;font-size:9pt;line-height:normal;text-indent:24px;margin-top:10px;word-wrap : break-word ;word-break : break-all ;font-size:16px;" onload="this.style.overflowX='auto';">
                                ...
                                <div id="res-btn" style="width:100%">
                                    @if(Auth::id() != $post->user_id && Auth::check() && $post->status == 0 && $date
                                    != '已经过期')
                                        <a href="{{action('DealSonController@confirm',$post->id)}}"><button
                                    ...
                                    @endif
                                        ...
                                        @endif
                                        <a href="{{action('TopicController@publish',['type'=>2,'id'=>$post->id])
                                        }}"><button
                                                    class="res-btn_son
                                        btn-n">我要投诉</button></a>
                                </div>
        ...
        <div class="owner">
                ...
                <div class="owner_detail">
                    <div class="owner_user_headpic">
                        <a href="/personal/{{$v->user->id}}">
                            <img src="{{$v->user->avatar}}"  width="90" height="90" style="border-radius:45px;" class="owner_user_img">
                        </a>
                        <div class="owner_user_name">
                            <a href="/personal/{{$v->user->id}}">{{ $v->user->name }}<span
                                        id="louzhu">[交易者]</span></a>
                            <br>
                            @if(Auth::id() != $v->user->id)

                                <a @if(Auth::check()) login="true" @else login="false" @endif href='javascript:;' class='gzuser gzuser{{ $v->user->id }}' follow_id="{{ $v->user->id }}">{!! Auth::check()&&Auth::user()->followed_user($v->user->id) ? "<span class='glyphicon glyphicon-minus'></span><span style='margin-left:5px'>已关注</span>" : "<span class='glyphicon glyphicon-plus'></span><span style='margin-left:5px'>关注他</span>" !!}</a>

                                <a href="/message/check/{{$v->user->id}}">
                                    <span class='glyphicon glyphicon-envelope' style="margin-left:2px;margin-right:5px;font-size:14px;"></span>私信
                                </a>
                            @endif
                        </div>
                    ...
                    <div class="owner_user_detail">性别 : @if($v->user->detail->sex == 0)女 @else 男 @endif</div>
                    @if($v->user->birthday != 0)
                        <div class="owner_user_detail">生日 : {{$v->user->birthday}}</div>
                    @endif

                    @if($v->user->email != '0')
                        <div class="owner_user_detail">邮箱 : {{$v->user->detail->email}}</div>
                    @endif

                    <div class="owner_user_detail">发帖次数 : {{$v->user->posts->count()}}</div>
                    <div class="owner_user_detail">回帖次数 : {{$v->user->replies->count()}}</div>
                    <div class="owner_user_detail">注册日期 : {{$v->user->created_at}}</div>
                    <div class="owner_user_detail">信用积分 : <a href="{{action('DealController@credit',$v->user->id)}}" target="_blank">点击查看</a></div>
                    <div class="owner_user_detail">交易次数 : {{$v->user->detail->transactionTimes}}</div>
                    <div class="owner_user_detail">交易金额 : {{$v->user->detail->transactionAmount}}</div>
                    <div class="owner_user_detail">好评率  : {{$v->user->mark->appreciation}}</div>
                    <div class="owner_user_detail">
                        好评：{{$v->user->mark->good}}
                        中评：{{$v->user->mark->commonly}}
                        差评：{{$v->user->mark->bad}}
                    </div>
                </div>
                <div class="owner_content">
                    ...
                    <div class="user_detail_x">
                        <div class="user_detail_x_line">认证状态 :
                            @if($v->user->card->status == 3)
                                <span>{{$v->user->role->role}} <span class="rzzt">「此用户已通过实名认证，地址，银行，姓名均认证成功」</span></span>
                            @else
                                <span>{{$v->user->role->role}} <span class="rzzt">「此用户还未实名认证」</span></span>
                            @endif
                        </div>

                        @if($v->user->card->realName != '0')
                            <div class="user_detail_x_line">姓 名 : <span>{{$v->user->card->realName}}</span></div>
                        @endif

                        @foreach($v->user->addresses as $k => $v)
                            <div class="user_detail_x_line">地 址{{$k+1}} : <span> {{$v['province']}}{{$v['city']}}{{$v['county']}}{{$v['street']}} </span></div>
                        @endforeach

                        <div class="user_detail_x_line">手 机 : <span>{{$v->user->phone}}

					</span></div>


                        @foreach( $v->user->banks as $k1 => $v1)
                            <div class="user_detail_x_line">银 行{{$k1+1}} : <span>{{$v1['cateBank']}}&nbsp; &nbsp;
                                    {{$v1['bankId']}} &nbsp; &nbsp; {{$v1['bankName']}}  &nbsp; &nbsp;
                                    {{$v1['tel']}}</span></div>
                        @endforeach

                        @if($v->user->detail->alipay != '0')
                            <div class="user_detail_x_line">支付宝 : <span>{{$v->user->detail->alipay}}</span></div>
                        @endif

                        @if($v->user->detail->vx != '0')
                            <div class="user_detail_x_line">微 信 : <span>{{$v->user->detail->vx}}</span></div>
                        @endif

                        @if($v->user->detail->qq != '0')
                            <div class="user_detail_x_line"> Q Q : <span>{{$v->user->detail->qq}}</span></div>
                        @endif
            ...
            <div class="owner_tips">
                <span class="owner_time">{{$v->created_at}}</span>
                <span class="owner_tip">免责声明及风险提示： 所有交易人员，凡未采用本站中介交易的，被骗后果自负。</span>
            </div>
        </div>
        @endforeach
                ...
                <div class="owner">
                    ...
                    <div class="owner_down">
                        <div class="owner_detail">
                            ...
                            <div class="owner_user_detail">回帖次数 : {{$v->user->replies->count()}}</div>
                            <div class="owner_user_detail">注册日期 : {{$v->user->created_at}}</div>
                            <div class="owner_user_detail">信用积分 : {{$v->user->detail->creditscore}}</div>
                            <div class="owner_user_detail">交易次数 : {{$v->user->detail->transactionTimes}}</div>
                            <div class="owner_user_detail">交易金额 : {{$v->user->detail->transactionAmount}}</div>
                            <div class="owner_user_detail">好评率  : {{$v->user->mark->appreciation}}</div>
                            <div class="owner_user_detail">
        ...
        @foreach($mark as $k => $v)
            <div class="owner">
                ...
                <div class="owner_down">
                    <div class="owner_detail">
                        <div class="owner_user_headpic">
                            <a href="/personal/{{$v->fromuser->id}}">
                                <img src="{{$v->fromuser->avatar}}"  width="90" height="90" style="border-radius:45px;" class="owner_user_img">
                            </a>
                            <div class="owner_user_name">
                                <a href="/personal/{{$v->fromuser->id}}">{{ $v->fromuser->name }}<span
                                            id="louzhu">[交易者]</span></a>
                                <br>
                                @if(Auth::id() != $v->fromuser->id)

                                    <a @if(Auth::check()) login="true" @else login="false" @endif href='javascript:;'
                                       class='gzuser gzuser{{ $v->fromuser->id }}' follow_id="{{ $v->fromuser->id
                                       }}">{!! Auth::check()&&Auth::user()->followed_user($v->fromuser->id) ? "<span class='glyphicon glyphicon-minus'></span><span style='margin-left:5px'>已关注</span>" : "<span class='glyphicon glyphicon-plus'></span><span style='margin-left:5px'>关注他</span>" !!}</a>

                                    <a href="/message/check/{{$v->fromuser->id}}">
                                        <span class='glyphicon glyphicon-envelope' style="margin-left:2px;margin-right:5px;font-size:14px;"></span>私信
                                    </a>
                                @endif
                            </div>
                        ...
                        <div class="owner_user_detail">
                            身份：{{$v->fromuser->role->role}}
                        </div>
                        <div class="owner_user_detail">性别 : @if($v->fromuser->detail->sex == 0)女 @else 男 @endif</div>
                        @if($v->fromuser->birthday != 0)
                            <div class="owner_user_detail">生日 : {{$v->fromuser->birthday}}</div>
                        @endif

                        @if($v->fromuser->email != '0')
                            <div class="owner_user_detail">邮箱 : {{$v->fromuser->detail->email}}</div>
                        @endif

                        <div class="owner_user_detail">发帖次数 : {{$v->fromuser->posts->count()}}</div>
                        <div class="owner_user_detail">回帖次数 : {{$v->fromuser->replies->count()}}</div>
                        <div class="owner_user_detail">注册日期 : {{$v->fromuser->created_at}}</div>
                        <div class="owner_user_detail">信用积分 : <a href="{{action('DealController@credit',$v->fromuser->id)}}" target="_blank">点击查看</a></div>
                        <div class="owner_user_detail">交易次数 : {{$v->fromuser->detail->transactionTimes}}</div>
                        <div class="owner_user_detail">交易金额 : {{$v->fromuser->detail->transactionAmount}}</div>
                        <div class="owner_user_detail">好评率  : {{$v->fromuser->mark->appreciation}}</div>
                        <div class="owner_user_detail">
                            好评：{{$v->fromuser->mark->good}}
                            中评：{{$v->fromuser->mark->commonly}}
                            差评：{{$v->fromuser->mark->bad}}
                        </div>
                    </div>
                    <div class="owner_content">
                        <div class="res" style="min-height:200px;font-size:9pt;line-height:normal;text-indent:24px;margin-top:10px;word-wrap : break-word ;word-break : break-all ;font-size:16px;" onload="this.style.overflowX='auto';">
                        ...
                        <div class="user_detail_x">
                            <div class="user_detail_x_line">认证状态 :
                                @if($v->fromuser->card->status == 3)
                                    <span>{{$v->fromuser->role->role}} <span class="rzzt">「此用户已通过实名认证，地址，银行，姓名均认证成功」</span></span>
                                @else
                                    <span>{{$v->fromuser->role->role}} <span class="rzzt">「此用户还未实名认证」</span></span>
                                @endif
                            </div>

                            @if($v->fromuser->card->realName != '0')
                                <div class="user_detail_x_line">姓 名 : <span>{{$v->fromuser->card->realName}}</span></div>
                            @endif

                            @foreach($v->fromuser->addresses as $k2 => $v2)
                                <div class="user_detail_x_line">地 址{{$k2+1}} : <span>
                                        {{$v2['province']}}{{$v2['city']}}{{$v2['county']}}{{$v2['street']}}
                                    </span></div>
                            @endforeach

                            <div class="user_detail_x_line">手 机 : <span>{{$v->fromuser->phone}}

					</span></div>


                            @foreach( $v->fromuser->banks as $k1 => $v1)
                                <div class="user_detail_x_line">银 行{{$k1+1}} : <span>{{$v1['cateBank']}}&nbsp; &nbsp;
                                        {{$v1['bankId']}} &nbsp; &nbsp; {{$v1['bankName']}}  &nbsp; &nbsp;
                                        {{$v1['tel']}}</span></div>
                            @endforeach

                            @if($v->fromuser->detail->alipay != '0')
                                <div class="user_detail_x_line">支付宝 : <span>{{$v->fromuser->detail->alipay}}</span></div>
                            @endif

                            @if($v->fromuser->detail->vx != '0')
                                <div class="user_detail_x_line">微 信 : <span>{{$v->fromuser->detail->vx}}</span></div>
                            @endif

                            @if($v->fromuser->detail->qq != '0')
                                <div class="user_detail_x_line"> Q Q : <span>{{$v->fromuser->detail->qq}}</span></div>
                            @endif
        ...
        <div id="match" style="margin-top:20px;" >
            ...
            <div class="other-body">
                <div class="other-body_left other-body_son" style="border-right: 1px solid #F0F0F0;">
                    <div style="  text-align: center;"> {{$post->cate->name}}最新买卖盘 <a style="color:#ff6600;
                    font-size:14px" href="{{action('DealSonController@trade',['id'=>$post->cate->id])}}" target="_blank">更多</a></div>
                    <div class="other-body_son_body">
                ...
                <div class="other-body_right other-body_son">
                    <div style="  text-align: center;"> {{$post->cate->name}}最新买成交   <a style="color:#ff6600;
                    font-size:14px" href="{{action('DealSonController@trade',['id'=>$post->cate->id])}}" target="_blank">更多</a></div>
                    <div class="other-body_son_body">
        ...
        <div style="float:left;width:100%;padding:0px;">
            @if(Auth::check() && Auth::user())
                <div id="plArticle">
                    <div>
                        <form id="form" class="form-horizontal" action="{{ action('DealSonController@message') }}"
                              method="post">
                            {{ csrf_field() }}

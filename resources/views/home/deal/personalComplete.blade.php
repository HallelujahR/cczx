...
<div style="padding:0px;" class="container-fluid">
    <div id="personal_con" style="width:100%;" newsType="complete" userid="{{Auth::id()}}" counts="{{ $counts }}">
        <!-- 左栏 -->
        ...
        <div id="personal_right" style="width:81%;">
        	<div class="layui-tab" lay-filter="news">
			  <ul class="layui-tab-title">
                ...
                <li style="float:left;width:100%;letter-spacing:3px;"><span style="letter-spacing:0px;margin-right:5px;color:red;">{{ $user->name }}</span>等待评分买卖盘</li>
			  </ul>
                        ...
                        <table class="layui-table">
                          ...
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td>@if($deal->status == 4) 交易完成，已互相打分@elseif($deal->status == 3)交割失败@endif</td>
                              <td>
                                @if($deal->status == 3)
                                      <a href="{{ action('RevokeController@release',$deal->id) }}" target="_blank" style="color:blue;">重新发布</a>
                                  @else
                                      您给对方评了：{{$deal['mark']['mark']}}分
                                  @endif
                              </td>
